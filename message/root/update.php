...
<?php
  ...
  if ($_GET['action'] == "热门内容") { //判断地址栏参数action的值是否等于update
    $sqlstr = "select * from tb_liu where id = " . $_GET['id']; //定义查询语句
...
if($_GET['action'] == "图文分享"){
    $sqlstr = "select * from tb_tu where id = " . $_GET['id']; //定义查询语句
    $result = mysqli_query($connID, $sqlstr); //执行查询语句
    $rows = mysqli_fetch_row($result); //将查询结果返回为数组
    
    ?>
    <form name="intFrom" method="post" action="update_ok.php">
      用户名：<input type="text"  class="txt" name="username" value="<?php echo $rows[1] ?>"></br></br>
      图片地址：<input type="text"  class="txt" name="src" value="<?php echo $rows[2] ?>"></br></br>
      留言内容：<input type="text"  class="txt" name="liuyan" value="<?php echo $rows[3] ?>"></br></br>
      <input type="hidden" name="action" value="update">
      <input type="hidden" name="nei" value="<?php echo $_GET['action'] ?>">
      <input type="hidden" name="id" value="<?php echo $rows[0] ?>">
      <input type="submit" name="Submit" class="btn btn-primary btn-lg" value="修改">
